...
EKF::EKF(const std::string& config_file)
    : is_time_set(false), is_initialized(false), imu_flag(false), gps_flag(false), time_init(0), config(config_file) {
...
void NavSol::SetNavSol(double& t, Eigen::Vector3d& orig, Eigen::Vector3d& pos, Eigen::Vector3d& vel,
                       Eigen::Quaterniond& att, Eigen::Vector3d& ba, Eigen::Vector3d& bg) {
    timestamp = t;
    origin = orig;
    position = pos;
    velocity = vel;
